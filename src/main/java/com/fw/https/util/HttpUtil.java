...
public class HttpUtil {
	...
	public static String processRequest(String q, String clientID) throws IOException {
		RequestStatus requestStatus = new RequestStatus();
		requestStatus.setTimestamp(System.currentTimeMillis());
		requestStatus.setStatus(true);
		boolean response = QueueManager.addNewRequest(requestStatus,clientID);
		if(System.currentTimeMillis()-requestStatus.getTimestamp()>5000)
			return "request took more than 5 seconds";
		if(response) {
			return "Accessed by HTTPS protocol....... : "+getRequest(q);
		}
